...
#if defined(NDEBUG)
#error "BinJit cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubnj) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateBNJSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating BNJ supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating BNJ supply..."), percent);
		}
...
{
    util::ThreadRename("binjit-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> bnjInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                bnjInputs.push_back(stakeIn);
        }
        const bool hasBNJInputs = !bnjInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasBNJInputs) {
                    ...
                    for (const CTxIn& bnjIn : bnjInputs)
                        if(bnjIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasBNJInputs) {
                            for (const CTxIn& stakeIn : bnjInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // BinJit: We use certain sporks during IBD, so check to see if they are
