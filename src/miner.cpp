...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("BinJit CoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("BinJit CoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("BinJit CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("binjit-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BinJit CoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("BinJit CoinMiner exception");
    } catch (...) {
        LogPrintf("BinJit CoinMiner exception");
    }

    LogPrintf("BinJit CoinMiner exiting\n");
}
