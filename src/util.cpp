...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\BinJit
// Windows >= Vista: C:\Users\Username\AppData\Roaming\BinJit
// Mac: ~/Library/Application Support/BinJit
// Unix: ~/.binjit
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BNJCoin";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "binjit.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty binjit.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL)
        {
            std::string strHeader = "addnode=45.77.175.92\n"
				    "addnode=207.148.70.103\n"
				    "addnode=139.59.1.32\n";
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override binjit.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "binjitd.pid"));
