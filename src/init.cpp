...
#include "config/binjit-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    util::ThreadRename("binjit-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "binjit.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "binjitd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 8018, 51474));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all BinJit specific functionality (Masternodes, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in BNJ) allowed by GUI and RPC for the stake split threshold (default: %s)"),
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:8018"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The BinJit developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("binjit-loadblk");
...
/** Sanity checks
 *  Ensure that BinJit is running in a usable environment with all
...
bool AppInit2()
{
 ...
 /*   if (GetBoolArg("-masternode", false) && !GetBoolArg("-listen", true))
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. BinJit is shutting down."));
...
#endif
    // Make sure only a single BinJit process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. BinJit is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // BinJit: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BinJit") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart BinJit to complete") << "\n";
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
            if (prev_version <= FEATURE_PRE_BinJit && pwalletMain->IsLocked()) {
        ...
        if (fFirstRun) {
            if (!fLegacyWallet) {
                ...
                pwalletMain->SetMaxVersion(FEATURE_PRE_BinJit);
            }
