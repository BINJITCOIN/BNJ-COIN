...
bool CBNJStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CBNJStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CBNJStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CBNJStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CBNJStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CBNJStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CBNJStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CBNJStake::GetUniqueness() const
{
    //The unique identifier for a BNJ stake is the outpoint
...
CBlockIndex* CBNJStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CBNJStake::ContextCheck(int nHeight, uint32_t nTime)
{
