...
#include "qt/binjit/splash.h"
#include "qt/binjit/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("binjit-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in binjit.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in binjit.qrc)
...
/** Class encapsulating BinJit startup and shutdown.
...
/** Main BinJit application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (BinJitGUI)
...
{
    window = new BinJitGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &BinJitGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &BinJitGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(BinJitGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(BinJitGUI::DEFAULT_WALLET);
        ...
        // BinJit: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BinJitGUI::handlePaymentRequest);
        connect(window, &BinJitGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. BinJit Coin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(binjit_locale);
    Q_INIT_RESOURCE(binjit);
    ...
    /// 6. Determine availability of data directory and parse binjit.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("BinJit Coin"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("BinJit Coin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // binjit: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("BinJit Coin didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
