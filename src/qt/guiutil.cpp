...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter BinJit Coin address (e.g. %1)").arg("M6tGR83SQbie1SW72hjcWJvFfip5krte2Z"));
...
{
    // return if URI is not valid or is no BinJit: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::BNJ, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert binjit:// to binjit:
    //
    //    Cannot handle this later, because binjit:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BNJ, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BinJit.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BinJit.lnk
...
{
    return GetAutostartDir() / "binjit.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a binjit.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BinJit Coin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the binjit app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add binjit app to startup item list
