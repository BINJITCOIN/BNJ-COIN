...
#include "qt/binjit/forms/ui_snackbar.h"
#include "qt/binjit/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(BinJitGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &BinJitGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
