...
#include "qt/binjit/dashboardwidget.h"
#include "qt/binjit/forms/ui_dashboardwidget.h"
#include "qt/binjit/sendconfirmdialog.h"
#include "qt/binjit/txrow.h"
...
DashboardWidget::DashboardWidget(BinJitGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareBNJ, "square-chart-bnj");
    setCssProperty(ui->labelBNJ, "text-chart-bnj");
    ...
    setCssProperty(ui->labelAmountBNJ, "text-stake-bnj-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &BinJitGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &BinJitGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &BinJitGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            bnj = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalBNJ += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesBNJ.append(bnj);

        int max = std::max(bnj, bnj);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesBNJ);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalBNJ > 0) {
        setCssProperty(ui->labelAmountBNJ, "text-stake-bnj");
    } else {
        setCssProperty(ui->labelAmountBNJ, "text-stake-bnj-disable");
    }
    forceUpdateStyle({ui->labelAmountBNJ});
    ui->labelAmountBNJ->setText(GUIUtil::formatBalance(chartData->totalBNJ, nDisplayUnit));
