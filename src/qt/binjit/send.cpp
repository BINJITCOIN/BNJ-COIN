...
#include "qt/binjit/forms/ui_send.h"
#include "qt/binjit/addnewcontactdialog.h"
#include "qt/binjit/qtutils.h"
#include "qt/binjit/sendchangeaddressdialog.h"
#include "qt/binjit/optionbutton.h"
#include "qt/binjit/sendconfirmdialog.h"
#include "qt/binjit/myaddressrow.h"
#include "qt/binjit/guitransactionsutils.h"
...
SendWidget::SendWidget(BinJitGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-bnj");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination bnjAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(bnjAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(bnjAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
