...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BNJ);
    ...
    if (settings.contains("nAnonymizeBinJitAmount") || reset)
        SoftSetArg("-anonymizebinjitamount", settings.value("nAnonymizeBinJitAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in binjit.cpp during shotdown to also remove the window positions
