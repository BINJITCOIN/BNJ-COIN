...
The build script we'll be using is contained in the BinJit github repository ([contrib/gitian-build.py](https://github.com/binjit-project/binjit/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the BinJit repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/BinJit-Project/BinJit/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, binjit-detached-sigs, and binjit GitHub repos.
