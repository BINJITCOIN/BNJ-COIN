...
*Setup instructions for a gitian build of BinJit using a VM or physical system.*

Gitian is the deterministic build process that is used to build the BinJit
...
to the BinJit GitHub release page.
...
- [Building BinJit](#building-binjit-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/binjit-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/binjit-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/binjit-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/binjit-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/binjit-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/binjit-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/binjit-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@binjit.trade) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/binjit-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/binjit-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/binjit-osx-*-build.assert
```
